...
export default async function StudentDashboard() {
  ...
  const levelId = user.unsafeMetadata?.level as string | undefined;
  if (!levelId) redirect("/onboarding");

  const levelData = LEVELS[levelId];
  if (!levelData) redirect("/onboarding");

  const isArabic = levelData.department === "arabic";

  // Upcoming calendar events (non-study, future)
    ...
    <div className="min-h-[calc(100vh-80px)] p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <ScheduleCard
            title="جدول يوم الجمعة"
            icon="📅"
            slots={FRIDAY_SCHEDULE}
          />
          <ScheduleCard
            title="جدول يوم السبت"
            icon="📅"
            slots={SATURDAY_SCHEDULE}
          />
        </div>
